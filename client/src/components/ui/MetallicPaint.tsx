...
export default function MetallicPaint({
  ...
  useEffect(() => {
    function initShader() {
      ...
      context.shaderSource(fragmentShader, liquidFragSource);
      context.compileShader(fragmentShader);

      if (!context.getShaderParameter(fragmentShader, context.COMPILE_STATUS)) {
        console.error("Fragment shader compilation error:", context.getShaderInfoLog(fragmentShader));
        return;
      }
      ...
      setUniforms(uniformLocations as Record<string, WebGLUniformLocation>);

      // Set static uniforms
      context.uniform1i(uniformLocations.u_image_texture, 0);
      context.uniform1f(uniformLocations.u_img_ratio, imageData.width / imageData.height);
    }
  ...
  useEffect(() => {
    ...
    if (gl && Object.keys(uniforms).length > 0) {
      requestAnimationFrame(render);
    }
  }, [gl, uniforms, params]);
